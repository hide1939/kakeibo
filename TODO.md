...
- 収支を登録したら画面遷移することなく欄に項目と金額が追加される+合計収支も更新される
    - 合計収支もReactを使って表示するようにしたい
        - まずこれ(やらないとpostしても表示が変わらない)
        - form.jsとtable.jsを統合する？その方が都合が良さそう
        - Reactのcomponentってどのくらいの粒度？
        - propsの仕組み学ぶ
    - 入力してボタンを押した際にreact側でフォームから入力値を取得してaxiosでpostリクエスト(onclickでメソッド指定、メソッドの中にpostのaxios) -> サーバー側に入力値を送信してDB登録(controllerは既に作っている)
    - 登録したデータをテーブルに追加して表示させる+合計収支も更新する 
